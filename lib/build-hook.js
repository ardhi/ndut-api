...
const build = async function (files, hook) {
  ...
  for (const f of files) {
    ...
    if (bases[1]) {
      _.each(bases[1].split(','), b => {
        const nkey = key + '@' + pascalCase(b)
        if (!hook[nkey]) hook[nkey] = []
        hook[nkey].push(mod)
      })
    } else {
...
module.exports = async function () {
  ...
  for (const n of config.nduts) {
    ...
    const files = await fastGlob(`${cfg.dir}/ndutApi/hook/{${bases}}*.js`)
